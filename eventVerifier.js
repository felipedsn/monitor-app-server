...
function verifyIdleness() {
	console.log("Verifying idleness");
	...
	if(!isBetweenSleepTime()) {
		sensorsRepository.getLastMovementRegistry(function(err, post) {
			if(err) {
				console.log("Some error happened when verifying idleness: " + err.message);
			} else if (post !== null) {
	      		if(itsBeenMoreThanInterval(post.createdDate, CONSTANTS.IDLENESS_INTERVAL)) {
	      			console.log("Idlness detected, sending push notification");
		...
		console.log("Monitored is in sleep time ");
	}
}

function verifyMedicineTime() {
	console.log("Verifying medicine time");
	...
	sensorsRepository.getLastMedicineRegistry(function(err, post) {
		if(err) {
				console.log("Some error happened when verifying medicine time: " + err.message);
		} else if (post !== null) {
      		var dateNow = moment();
      		...
      		if(moment(post.createdDate).isSame(moment(), 'day')) {
	      		...
	      		if(diffLastMedicineInSeconds >= CONSTANTS.PRE_MEDICINE_INTERVAL) {
	      			if(diffNowInSeconds >= CONSTANTS.POST_MEDICINE_INTERVAL) {
	      				console.log("No medicine detected, sending push notification");
...
function verifyEvents() {
	updateStates(function() {
		console.log("Event verifier triggered");
		verifyMedicineTime();
		if(isHome) {
			console.log("Monitored is in home");
			verifyIdleness();
		}
	});
}
